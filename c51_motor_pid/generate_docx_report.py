...
"""
=============================================================================
C51 Motor Speed PID Control System - Homework Word Document Generator (v3)
=============================================================================
Creates a clean, standard, academic-style report with simple and elegant
formatting, standard black titles, Three-Line Tables (三线表), and clean code.
=============================================================================
"""
...
def set_cell_shading(cell, color_hex):
    """Applies a light background shading to a table cell."""
    shading_elm = parse_xml(f'<w:shd {nsdecls("w")} w:fill="{color_hex}"/>')
    cell._tc.get_or_add_tcPr().append(shading_elm)

def set_cell_margins(cell, top=100, bottom=100, left=150, right=150):
    """Applies standard margins to a table cell."""
...
def set_three_line_table(table):
    """Sets a classic academic Three-Line Table (三线表) layout."""
    ...
    # Set a bottom line for the first row (header row)
...
def make_simple_callout(doc, text_content):
    """Generates a clean callout box with thin borders and a light background."""
...
def build_report():
    ...
    COLOR_TEXT = RGBColor(0, 0, 0) # Black for true academic simplicity
    ...
    run_sub = p_sub.add_run("课程名称：单片机原理及应用 / 课程设计作业")
    run_sub.font.name = u"宋体"
    run_sub.font.size = Pt(11)
    run_sub.font.color.rgb = COLOR_TEXT
    ...
    r_h1.bold = True
    r_h1.font.color.rgb = COLOR_TEXT
    ...
    r_req = p_req.add_run("系统核心功能与设计指标：")
    r_req.font.name = u"宋体"
    r_req.bold = True
    r_req.font.size = Pt(10.5)
    r_req.font.color.rgb = COLOR_TEXT
    ...
    r_h2.bold = True
    r_h2.font.color.rgb = COLOR_TEXT
...
"""
    ...
    # Add ASCII block with a clean, simple border box
    ...
    tcPr.append(tcBorders)
    p_ascii = cell.paragraphs[0]
    p_ascii.paragraph_format.line_spacing = 1.05
    p_ascii.paragraph_format.space_after = Pt(0)
    r_ascii = p_ascii.add_run(ascii_schematic)
    r_ascii.font.name = "Consolas"
    r_ascii.font.size = Pt(8.5)
    r_ascii.font.color.rgb = COLOR_TEXT
    doc.add_paragraph().paragraph_format.space_after = Pt(6)
    ...
    for i, cell in enumerate(table.rows[0].cells):
        ...
        r_cell.bold = True
        r_cell.font.name = u"黑体"
        r_cell.font.size = Pt(10)
        r_cell.font.color.rgb = COLOR_TEXT
    ...
    r_h3.bold = True
    r_h3.font.color.rgb = COLOR_TEXT
    ...
    r_fig2.font.name = u"黑体"
    r_fig2.font.size = Pt(9.5)
    r_fig2.bold = True
    r_fig2.font.color.rgb = COLOR_TEXT
...
+------------+      ^              +-------+              +---------------+
                    ...
                    +------| Encoder Feedback|<-----------| L298N & Motor |
                           ...
                           +-----------------+            +---------------+
"""
    # Block Diagram inside simple light border
    block_table = doc.add_table(rows=1, cols=1)
    block_table.alignment = WD_TABLE_ALIGNMENT.CENTER
    block_table.autofit = False
    block_table.columns[0].width = Inches(5.8)
    cell = block_table.cell(0, 0)
    set_cell_shading(cell, "FAFAFA")
    set_cell_margins(cell, top=100, bottom=100, left=150, right=150)
    tcPr = cell._tc.get_or_add_tcPr()
    tcBorders = parse_xml(
    ...
    p_block.paragraph_format.line_spacing = 1.05
    p_block.paragraph_format.space_after = Pt(0)
    r_block = p_block.add_run(ascii_block_diagram)
    r_block.font.name = "Consolas"
    r_block.font.size = Pt(8.5)
    r_block.font.color.rgb = COLOR_TEXT
    doc.add_paragraph().paragraph_format.space_after = Pt(6)
    ...
    r_h3_sub1.font.name = u"黑体"
    r_h3_sub1.font.size = Pt(11.5)
    r_h3_sub1.bold = True
    r_h3_sub1.font.color.rgb = COLOR_TEXT
    ...
    r_h3_sub2.font.name = u"黑体"
    r_h3_sub2.font.size = Pt(11.5)
    r_h3_sub2.bold = True
    r_h3_sub2.font.color.rgb = COLOR_TEXT
    ...
    r_f1.font.color.rgb = COLOR_TEXT
    ...
    r_f2.font.color.rgb = COLOR_TEXT

    make_simple_callout(doc, "【核心算法防暴走技巧——积分抗饱和机制 (Anti-Windup)】\n"
    ...
    r_h4.bold = True
    r_h4.font.color.rgb = COLOR_TEXT
    
    p = doc.add_paragraph()
    r = p.add_run("以下是完整的系统控制程序源码。程序基于标准 C51 开发，可直接在 Keil uVision 开发环境中建立工程、编译生成 .hex 固件文件：")
    r.font.name = u"宋体"
    r.font.size = Pt(10.5)
    r.font.color.rgb = COLOR_TEXT

    # Code block inside a simple gray bordered table
    code_table = doc.add_table(rows=1, cols=1)
    code_table.alignment = WD_TABLE_ALIGNMENT.CENTER
    code_table.autofit = False
    code_table.columns[0].width = Inches(5.8)
    ...
    set_cell_shading(code_cell, "FAFAFA") # Light white/gray background
    set_cell_margins(code_cell, top=120, bottom=120, left=150, right=150)
    ...
    r_code.font.color.rgb = COLOR_TEXT
    ...
    r_h5.bold = True
    r_h5.font.color.rgb = COLOR_TEXT
    ...
    steps = [
        ...
        "第二步（MOTOR-ENCODER 电机属性配置）：双击 Proteus 电路图中的电机元件，弹出编辑窗口。将电机的“Encoder Pulses Per Revolution”（每转脉冲数）属性设置为 30，将“Nominal Voltage”（额定电压）设置为 12V。注意：这里的脉冲数直接决定了程序计算时速的系数，软件代码中基于 30 线编码器和 200ms 采样时间窗对计算参数进行了完美的对应（RPM = 脉冲数 * 10）。",
        "第三步（Keil 固件生成）：在 Keil uVision 开发环境中新建一个标准 C51 工程，选择 AT89C51 芯片。将 main.c 文件添加到工程中。点击 Project -> Options for Target -> Output，勾选 “Create HEX File”（创建 HEX 格式烧录文件）。点击 Build 进行编译，生成 main.hex 文件。",
        ...
        "第五步（闭环 PID 转速稳定运行）：单片机启动后，LCD1602 会立刻初始化并显示“Set Speed:100RPM”，表示初始设定目标转速为 100 RPM。电机迅速旋转，由于初始状态转速为零，PID控制算法中比例 P 项和积分 I 项迅速发挥效果，误差 error 减小，PWM 占空比 pwm_duty 稳定调节到电机所需的驱动强度。仅经过约 0.8 秒左右的轻微修正，实际转速 Cur Speed 会极其精准地静止并锁定在 “100RPM”！",
        "第六步（多挡交互式调速测试）：点击 KEY_UP 按键，目标转速 Set Speed 增加 10 RPM 至 110 RPM。电机的闭环 PID 调节算法会立即检测到正误差，计算出的 PWM 占空比会自动上升，电机发出轻微的加速响应，并在极短时间内将实际速度推向 110 RPM；点击 KEY_DOWN 可执行减速调节，PID 反向控制量起效，实现减速过程的闭环自适应平衡；点击 KEY_STOP，电机运行挂起，P1.0 占空比瞬间清零，电机随阻力缓缓停止旋转，实际速度回零。"
    ]
    ...
    for s in steps:
        ...
        if len(parts) == 2:
            ...
            r_normal.font.color.rgb = COLOR_TEXT
        else:
